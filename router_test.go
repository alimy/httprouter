...
func TestRouter(t *testing.T) {
	...
	router.Handle("GET", "/user/:name", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		routed = true
		if p := ParamsFrom(r); p["name"] != "gopher" {
			t.Fatalf("wrong wildcard values: want %s, got %s", "gopher", p["name"])
		}
	}))
...
func TestRouterLookup(t *testing.T) {
	routed := false
	wantHandler := http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
		routed = true
	})
	...
	handler, _, tsr := router.Lookup("GET", "/nope")
	if handler != nil {
	...
	router.GET("/user/:name", wantHandler)
	...
	handler, _, tsr = router.Lookup("GET", "/user/gopher/")
	if handler != nil {
		t.Fatalf("Got handle for unregistered pattern: %v", handler)
	}
	if !tsr {
